...
public class PLView extends FragmentActivity implements PLIView, SensorEventListener,
        OnDoubleTapListener {
    /**
     * member variables
     */
    ...
    private boolean mIsAccelerometerEnabled, mIsAccelerometerLeftRightEnabled,
            mIsAccelerometerUpDownEnabled;
            ...
            if (panorama != null) {
                ...
                if (mRenderer != null) {
                    ...
                    mRenderer.setInternalListener(new PLRendererListener() {
                        @Override
                        public void rendererFirstChanged(GL10 gl, PLIRenderer render, int width,
                                int height) {
                            mGLContext = gl;
    ...
    public CGRect getRenderingViewport() {
        return (mRenderer != null ? mTempRenderingViewport.setValues(mRenderer.getViewport()) :
                mTempRenderingViewport.reset());
    }

    @Override
    public CGSize getRenderingSize() {
        return (mRenderer != null ? mTempRenderingSize.setValues(mRenderer.getSize()) :
                mTempRenderingSize.reset());
    }
    ...
    public void setAnimationFrameInterval(int animationFrameInterval) {
        if (animationFrameInterval >= 1) {
            ...
            mAnimationInterval =
                    PLConstants.kDefaultAnimationTimerIntervalByFrame * animationFrameInterval;
        }
    }
    ...
    public void setAccelerometerSensitivity(float accelerometerSensitivity) {
        mAccelerometerSensitivity = PLMath.valueInRange(accelerometerSensitivity,
                PLRange.PLRangeMake(PLConstants.kAccelerometerSensitivityMinValue,
        ...
        if (touches.size() == 2 && isZoomEnabled()) {
            ...
            if (mFovCounter < PLConstants.kDefaultFovMinCounter) {
                if (mFovCounter == PLConstants.kDefaultFovMinCounter - 1)
                    mFovDistance = PLMath.distanceBetweenPoints(mAuxiliarStartPoint,
                            mAuxiliarEndPoint);
                return false;
            }

            float distance = PLMath.distanceBetweenPoints(mAuxiliarStartPoint, mAuxiliarEndPoint)
            ...
            if (mListener != null)
                isNotCancelable = mListener.onShouldRunZooming(this, distanceDiff, isZoomIn,
                        !isZoomIn);
        ...
        if (mIsValidForFov) {
            ...
            if (!this.executeDefaultAction(touches, PLTouchEventType.PLTouchEventTypeEnded)) {
                ...
                if (mIsScrollingEnabled && listenerExists)
                    isNotCancelable = mListener.onShouldBeingScrolling(this, mStartPoint,
                            mEndPoint);

                if (mIsScrollingEnabled && isNotCancelable) {
                    boolean isValidForMoving = (PLMath.distanceBetweenPoints(mStartPoint,
                            mEndPoint) >= mMinDistanceToEnableScrolling);
                    if (mIsInertiaEnabled) {
                        if (isValidForMoving) {
                            ...
                            if (listenerExists) {
                                ...
                                isNotCancelable = mListener.onShouldBeginInertia(this,
    ...
    protected void inertia() {
        ...
        float b =
                (mStartPoint.y * mEndPoint.x - mEndPoint.y * mStartPoint.x) / (mEndPoint.x - mStartPoint.x);
        float x, y, add;
    ...
    protected boolean activateAccelerometer() {
        if (mSensorManager != null && mSensorManager.registerListener(this,
                ...
                (int) (mAccelerometerInterval * 1000.0f)))
            return true;
        PLLog.debug("PLView::activateAccelerometer", "Accelerometer sensor is not available on " +
                "the device!");
        return false;
    }
    ...
    protected void accelerometer(SensorEvent event, UIAcceleration acceleration) {
        ...
        if (mIsAccelerometerEnabled) {
            ...
            float x = 0.0f, y = (mIsAccelerometerUpDownEnabled ? -acceleration.z : 0.0f), factor
                    = mAccelerometerSensitivity * (mPanorama.getCamera().isReverseRotation() ?
                    -PLConstants.kAccelerometerMultiplyFactor :
                    PLConstants.kAccelerometerMultiplyFactor);
            ...
            mAuxiliarEndPoint.setValues(mAuxiliarStartPoint.x + x * factor,
                    mAuxiliarStartPoint.y + y * factor);
    ...
    protected boolean activateGyroscope() {
        return (mSensorManager != null && mSensorManager.registerListener(this,
                mSensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE),
                (int) (PLConstants.kDefaultGyroscopeInterval * 1000.0f)));
    }
    ...
    protected boolean activateMagnetometer() {
        return (mSensorManager != null && mSensorManager.registerListener(this,
                ...
                (int) (PLConstants.kDefaultMagnetometerInterval * 1000.0f)));
    }
    ...
    public boolean startSensorialRotation() {
        if (!mIsValidForSensorialRotation) {
            if (this.activateGyroscope()) {
                ...
                PLLog.debug("PLView::startSensorialRotation", "Gyroscope sensor is not available " +
                        "on device!");
                if (mSensorManager != null && mSensorManager.getSensorList(Sensor.TYPE_ACCELEROMETER).size() > 0 && mSensorManager.getSensorList(Sensor.TYPE_MAGNETIC_FIELD).size() > 0) {
                    ...
                    mSensorialRotationType =
                            PLSensorialRotationType.PLSensorialRotationTypeAccelerometerAndMagnetometer;
                    mIsValidForSensorialRotation = true;
                    this.activateMagnetometer();
                } else
                    PLLog.debug("PLView::startSensorialRotation", "Accelerometer or/and " +
                            "magnetometer sensor/s is/are not available on device!");
            }
            return mIsValidForSensorialRotation;
        }
        return false;
    ...
    protected void updateGyroscopeRotationByOrientation(UIDeviceOrientation currentOrientation,
    ...
    protected boolean activateOrientation() {
        if (mSensorManager != null && mSensorManager.registerListener(this,
            ...
            return true;
        PLLog.debug("PLView::activateOrientation", "Orientation sensor is not available on the " +
                "device!");
        return false;
    }
                ...
                (
                        new PLTransitionListener() {
                            ...
                            public void didBeginTransition(PLITransition transition) {
                                if (mListener != null)
                                    mListener.onDidBeginTransition(transition.getView(),
                                            transition);
                            }
                            ...
                            public void didProcessTransition(PLITransition transition,
                                ...
                                if (mListener != null)
                                    mListener.onDidProcessTransition(transition.getView(),
                                            transition, progressPercentage);
                            }

                            @Override
                            public void didStopTransition(PLITransition transition,
                                    int progressPercentage) {
                                mIsValidForTransition = false;
                                mCurrentTransition = null;
                                if (mListener != null)
                                    mListener.onDidStopTransition(transition.getView(),
                                            transition, progressPercentage);
                            }
    ...
    public void load(PLILoader loader) {
        this.load(loader, false, null, PLConstants.kFloatUndefinedValue,
                PLConstants.kFloatUndefinedValue);
    }

    @Override
    public void load(PLILoader loader, boolean showProgressBar) {
        this.load(loader, showProgressBar, null, PLConstants.kFloatUndefinedValue,
                PLConstants.kFloatUndefinedValue);
    }

    @Override
    public void load(PLILoader loader, boolean showProgressBar, PLITransition transition) {
        this.load(loader, showProgressBar, transition, PLConstants.kFloatUndefinedValue,
                PLConstants.kFloatUndefinedValue);
    }

    @Override
    public void load(final PLILoader loader, boolean showProgressBar,
            final PLITransition transition, final float initialPitch, final float initialYaw) {
        if (loader != null) {
                ...
                handler.postDelayed
                        (
                                new Runnable() {
                                    @Override
                                    public void run() {
                                        loader.load(PLView.this, transition, initialPitch,
                                                initialYaw);
                                    }
                                },
                                300
...
//        int top = mLocation[1], left = mLocation[0];
        ...
        for (int i = 0, length = Math.min(event.getPointerCount(), kMaxTouches); i < 1; i++) {
            UITouch touch = mInternalTouches.get(i);
            touch.setPosition(4 * (event.getX() ), 4 * (event.getY() ));
//            touch.setPosition(4 * (event.getX(i) - left), 4 * (event.getY(i) - top));
...
//                    );
    ...
    protected View onGLSurfaceViewCreated(GLSurfaceView glSurfaceView) {
        ...
        mContentLayout.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT,
                LayoutParams.FILL_PARENT));
        mContentLayout.addView(glSurfaceView, new LayoutParams(LayoutParams.FILL_PARENT,
                LayoutParams.FILL_PARENT));
        LayoutParams progressBarLayoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT,
            ...
            case Sensor.TYPE_ORIENTATION:
                ...
                if (mCurrentDeviceOrientation != newOrientation) {
                    if (mIsValidForSensorialRotation && mSensorialRotationType == PLSensorialRotationType.PLSensorialRotationTypeGyroscope)
                        this.updateGyroscopeRotationByOrientation(mCurrentDeviceOrientation,
                                newOrientation);
                ...
                if (mIsRendererCreated && mRenderer.isRunning() && !mIsValidForTransition) {
                    if (mIsValidForSensorialRotation && mSensorialRotationType == PLSensorialRotationType.PLSensorialRotationTypeAccelerometerAndMagnetometer && mSensorialRotationAccelerometerData != null) {
                        if (mSensorialRotationThresholdFlag) {
                            SensorManager.getRotationMatrix(mSensorialRotationRotationMatrix,
                                    null, mSensorialRotationAccelerometerData, values);
                            SensorManager.remapCoordinateSystem(mSensorialRotationRotationMatrix,
                                    SensorManager.AXIS_X, SensorManager.AXIS_Z,
                                    mSensorialRotationRotationMatrix);
                            SensorManager.getOrientation(mSensorialRotationRotationMatrix,
                                    mSensorialRotationOrientationData);
                            float yaw =
                                    mSensorialRotationOrientationData[0] * PLConstants.kToDegrees;
                            float pitch =
                                    -mSensorialRotationOrientationData[1] * PLConstants.kToDegrees;
                ...
                if (mIsRendererCreated && mRenderer.isRunning() && !mIsValidForTransition) {
                    if (mHasFirstGyroscopePitch) {
                        if (mGyroscopeLastTime != 0) {
                            float timeDiff =
                                    (event.timestamp - mGyroscopeLastTime) * PLConstants.kGyroscopeTimeConversion;
                            if (timeDiff > 1.0)
                        ...
                        switch (mCurrentDeviceOrientation) {
                            ...
                            case UIDeviceOrientationLandscapeLeft:
                                ...
                                mGyroscopeRotationY =
                                        -cameraRotation.pitch * PLConstants.kToRadians;
                                break;
                            case UIDeviceOrientationLandscapeRight:
                                mGyroscopeRotationX = cameraRotation.yaw * PLConstants.kToRadians;
...
//                return true;
            switch (event.getAction()) {// & MotionEvent.ACTION_MASK
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if(oldMoveY == -1)
                        oldMoveY = event.getY();
                    if(Math.abs(oldMoveX - event.getX()) < 10 && Math.abs(oldMoveY - event.getY()) < 10){
